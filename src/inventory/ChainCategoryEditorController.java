...
public class ChainCategoryEditorController {
    ...
    private void loadCategories() {
        String sql = "SELECT category_name FROM ch_category";
    ...
    private void loadWeights() {
        String sql = "SELECT weight_name FROM ch_weight";
        try (Connection connect = database.connectDb(); PreparedStatement prepare = connect.prepareStatement(sql); ResultSet result = prepare.executeQuery()) {
            while (result.next()) {
    ...
    private void loadLengths() {
        String sql = "SELECT length_name FROM ch_length";
        try (Connection connect = database.connectDb(); PreparedStatement prepare = connect.prepareStatement(sql); ResultSet result = prepare.executeQuery()) {
            while (result.next()) {
    ...
    private void applyChanges() {
        ...
        for (String category : addedCategories) {
            String sql = "INSERT INTO ch_category (category_name) VALUES (?)";
            executeUpdate(sql, category);
        }
        for (String category : removedCategories) {
            String sql = "DELETE FROM ch_category WHERE category_name = ?";
            executeUpdate(sql, category);
        }

        // Apply weight changes
        for (String weight : addedWeights) {
            String sql = "INSERT INTO ch_weight (weight_name) VALUES (?)";
            executeUpdate(sql, weight);
        }
        for (String weight : removedWeights) {
            String sql = "DELETE FROM ch_weight WHERE weight_name = ?";
            executeUpdate(sql, weight);
        }

        // Apply length changes
        for (String length : addedLengths) {
            String sql = "INSERT INTO ch_length (length_name) VALUES (?)";
            executeUpdate(sql, length);
        }
        for (String length : removedLengths) {
            String sql = "DELETE FROM ch_length WHERE length_name = ?";
            executeUpdate(sql, length);
        }
